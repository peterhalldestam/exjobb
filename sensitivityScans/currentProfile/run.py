...
ds_init.solver.setBackupSolver(Solver.LINEAR_SOLVER_LU)
...
do_cond = runiface(ds_init, 'init/output_conductivity.h5', quiet=False)

""" Second intialization: set target current profile """
I_p = 15e6	# [A]
j_fun = lambda r, alpha, beta: (1 - alpha*(r/a)**2)**beta


alpha_list = [0.3, 0.6, 0.9]
beta_list = [0.2, 0.4, 0.6]
for alpha in alpha_list:
	for beta in beta_list:
		ds_curr = DREAMSettings(ds_init)

		j_prof = j_fun(rj0, alpha, beta)
		j_target = I_p * 2*np.pi * j_prof / do_cond.grid.integrate(j_prof)
		E = j_target / do_cond.other.fluid.conductivity[-1,:]

		ds_curr.eqsys.E_field.setPrescribedData(E, radius=do_cond.grid.r)

		subprocess.Popen(['rm','init/output_current.h5'])
		do = runiface(ds_curr, 'init/output_current.h5', quiet=False) # Using the same time step as before

		""" Main simulations: apply self-consistent temperature evolution """
		ds_main = DREAMSettings(ds_curr)
		...
		t_max = 160e-3
		nt = 24e3
		ds_main.timestep.setTmax(t_max)
		ds_main.timestep.setNt(nt)
		ds_main.timestep.setNumberOfSaveSteps(400)

		# Enable avalanche, hottail and Dreicer generation
		ds_main.eqsys.n_re.setAvalanche(Runaways.AVALANCHE_MODE_FLUID)
		ds_main.eqsys.n_re.setDreicer(Runaways.DREICER_RATE_NEURAL_NETWORK)

		ds_main.eqsys.f_hot.setInitialProfiles(rn0=rn0, n0=ne_profile, rT0=rT0, T0=T_init_profile)
		ds_main.eqsys.n_re.setHottail(Runaways.HOTTAIL_MODE_ANALYTIC_ALT_PC)

		# Enable self consistent evolution of E-field
		ds_main.eqsys.E_field.setType(ElectricField.TYPE_SELFCONSISTENT)
		ds_main.eqsys.E_field.setBoundaryCondition(ElectricField.BC_TYPE_SELFCONSISTENT, inverse_wall_time=0, R0=R0)

		# Enable self consistent temperature evolution
		ds_main.eqsys.T_cold.setType(Temperature.TYPE_SELFCONSISTENT)
		ds_main.eqsys.T_cold.setRecombinationRadiation(Temperature.RECOMBINATION_RADIATION_NEGLECTED)
		...
		# Run main simulation
		do = runiface(ds_main, f'output/output_alpha_{alpha}_beta_{beta}.h5', quiet=False)
